...
func FprintUsage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  radosgw-assume -p myprofile                    # Use specific profile, clean output")
	_, _ = fmt.Fprintln(w, "  radosgw-assume --env                           # Use environment variables")
	_, _ = fmt.Fprintln(w, "  radosgw-assume -d 2h -p myprofile              # 2-hour session duration")
	_, _ = fmt.Fprintln(w, "  radosgw-assume -d 30m -p myprofile             # 30-minute session duration")
	_, _ = fmt.Fprintln(w, "  radosgw-assume -d 15m -p myprofile             # 15-minute session duration (minimum)")
	_, _ = fmt.Fprintln(w, "  radosgw-assume -s my-session -p myprofile      # Custom session name")
	...
	_, _ = fmt.Fprintln(w, "  eval \"$(radosgw-assume)\"                     # Interactive export with eval")
	_, _ = fmt.Fprintln(w, "  eval \"$(radosgw-assume -p myprofile)\"        # Direct profile export with eval")
	_, _ = fmt.Fprintln(w, "  source <(radosgw-assume)                       # Interactive export with source")
	_, _ = fmt.Fprintln(w, "  source <(radosgw-assume -p myprofile)          # Direct profile export with source")
